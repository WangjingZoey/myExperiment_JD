...
public class WkfTagSet {
	//生成wkf的tag文档
			public static void main(String[] args) 
			{
 				try {
					int id = 1;
		            File file = new File("C:/Users/DJ/Desktop/1/evaluation/wkftag.xls"); // 创建文件对象
		            Workbook wb = Workbook.getWorkbook(file); // 从文件流中获取Excel工作区对象（WorkBook）
		            Sheet sheet = wb.getSheet(0); // 从工作区中取得页（Sheet） 
//		            List<String>src = new ArrayList<String>();
		            for (int i = 0; i < sheet.getRows(); i++) { // 循环打印Excel表中的内容 
		            	BufferedWriter writer1 = new BufferedWriter(
		    					new FileWriter(new File("C:/Users/DJ/Desktop/1/evaluation/wkftag/"+id+".txt")));
